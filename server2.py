...
from overall_prc import overall_demo
from super_res.sr import sr_overall
from sod.dfi import build_model
from overall_prc.interrogator import Interrogator
...
class build_model():
    def __init__(self):
        ...
        netscale = 4
        loadnet = torch.load('/home/compu/Downloads/StableDiffusion_Outpainting/RealESRGAN_x4plus.pth', map_location=torch.device('cpu'))
        if 'params_ema' in loadnet:
            keyname = 'params_ema'
        else:
            keyname = 'params'
        self.sr_model.load_state_dict(loadnet[keyname], strict=True)
        self.sr_model.eval()
        self.sr_model.to(device)

        # SD
        self.pipe = StableDiffusionInpaintPipeline.from_pretrained("/home/compu/Downloads/StableDiffusion_Outpainting/stable-diffusion-inpainting", safety_checker=StableDiffusionSafetyChecker.from_pretrained("/home/compu/Downloads/StableDiffusion_Outpainting/stable-diffusion-safety-checker"))
        self.pipe.to(device)

        # tagger
        self.interrogator = interrogators['wd14-convnextv2.v1']

        # SOD
        self.sod_model = build_model()
        self.sod_model.load_state_dict(torch.load('/home/compu/Downloads/StableDiffusion_Outpainting/sod/dfi.pth'))
        self.sod_model.eval()
        self.sod_model.to(device)
...
def process_image():
    ...
    if file and allowed_file(file.filename):
        ...
        if method == 'auto':
            processed_image = overall_demo.overall_prc(image)
